...
export default function WelcomeScreen({ onEnter }) {
  ...
  const handleShare = () => {
    ...
    const text = "¡Usá X-penses para llevar tus gastos compartidos! 💸";
    if (navigator.share) {
      navigator.share({ title: "X-penses", text, url });
    } else {
  ...
  return (
    <div style={{
      ...
      overflow: "hidden",
      position: "relative",
      // Safe area padding para iPhone con notch/Dynamic Island
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", padding:"0 24px", maxWidth:430, margin:"0 auto", width:"100%" }}>
        ...
        <div style={{ paddingTop: 40, opacity: visible ? 1 : 0, animation: visible ? "fadeUp 0.6s ease forwards" : "none", display:"flex", alignItems:"center", gap:14, marginBottom: 36 }}>
          <img
            src="/logo.png"
            alt="X-penses"
            style={{ width:54, height:54, borderRadius:16, boxShadow:"0 8px 24px rgba(0,0,0,0.4)", animation:"float 4s ease-in-out infinite" }}
          />
          <span style={{ fontSize:26, fontWeight:800, color:"#fff", letterSpacing:-0.5 }}>X-penses</span>
        </div>

        {/* ── HEADLINE ── */}
        <div style={{ opacity: visible ? 1 : 0, animation: visible ? "fadeUp 0.7s ease 0.05s forwards" : "none", marginBottom: 32 }}>
          <p style={{ fontSize:13, fontWeight:600, letterSpacing:2.5, textTransform:"uppercase", color:"#4F7FFA", margin:"0 0 14px" }}>
            Finanzas claras, cortito y al pie
          </p>
          <h1 style={{ fontSize:36, fontWeight:800, color:"#fff", margin:"0 0 14px", lineHeight:1.1, letterSpacing:-1.5 }}>
        ...
        <div style={{ opacity: visible ? 1 : 0, animation: visible ? "fadeUp 0.7s ease 0.15s forwards" : "none", marginBottom: 16 }}>
          <div style={{ position:"relative", minHeight:100, overflow:"hidden" }}>
            <div
              ...
              className={animDir === "in" ? "feature-slide-in" : "feature-slide-out"}
              style={{
              ...
              <div style={{ width:52, height:52, borderRadius:16, background:`${f.color}22`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:26, flexShrink:0 }}>
                {f.icon}
              </div>
              <div>
                <p style={{ margin:"0 0 4px", fontSize:16, fontWeight:700, color:"#fff" }}>{f.title}</p>
                <p style={{ margin:0, fontSize:13, color:"#ffffff77", lineHeight:1.4 }}>{f.desc}</p>
              </div>
            </div>
          </div>
          ...
          <div style={{ display:"flex", gap:6, justifyContent:"center", marginTop:14 }}>
            {FEATURES.map((_, i) => (
              <button key={i} onClick={() => goToFeature(i)} style={{
                width: activeFeature === i ? 22 : 7, height:7, borderRadius:4,
        ...
        <div style={{ opacity: visible ? 1 : 0, animation: visible ? "fadeUp 0.7s ease 0.25s forwards" : "none", paddingBottom: 40 }}>
          ...
          <button
            ...
            disabled={loading}
            style={{ width:"100%", padding:"16px 20px", borderRadius:18, background:"#fff", border:"none", cursor: loading ? "default" : "pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:12, fontFamily:SF, fontWeight:700, fontSize:16, color:"#1a1a2e", opacity: loading ? 0.7 : 1, marginBottom:12, transition:"opacity 0.2s", boxShadow:"0 4px 20px rgba(0,0,0,0.3)" }}>
